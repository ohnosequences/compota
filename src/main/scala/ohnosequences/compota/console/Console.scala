...
import ohnosequences.compota.{NisperoGraph, NisperoAux, Compota, Tasks}
import ohnosequences.logging.ConsoleLogger
import scala.xml.{Node, NodeSeq}
import com.amazonaws.services.autoscaling.model.AutoScalingGroup
import collection.JavaConversions._
import ohnosequences.compota.queues.{ProductQueue, MonoidQueueAux}
...
case class Console(compota: Compota, server: Server) {
...
//  <li><a href="/shutdown">shutdown</a></li>
  ...
  def nisperonInfo: NodeSeq = {
    <div class="page-header">
      <h1>
        {compota.compotaConfiguration.id}
      </h1>
    </div>
  }
...
//    </div>
  ...
  def nisperoInfo(nisperoName: String): NodeSeq = {
    ...
    compota.nisperoNames.get(nisperoName) match {
      ...
      case Some(nispero) =>
        ...
        <div class="page-header">
          <h1>
            {compota.compotaConfiguration.id}<small>
  ...
  //  val log = S3Logger.prefix(compota.compotaConfiguration, parent)
