...
import ohnosequences.compota.queues.S3QueueAbstract
...
with ServerErrorResponse {
  val aws =  console.compota.aws
  val as =  console.compota.aws.as.as
  ...
  def intent = Auth(users) {
    case GET(Path("/")) => {
      ...
      val mainPage = main().mkString
        ...
        .replace("$name$", console.compota.compotaConfiguration.id)

      HtmlCustom(mainPage)
    }

    case GET(Path("/undeploy")) => {
      console.compota.sendUndeployCommand("adhoc", force = true)
    ...
    case GET(Path(Seg("instanceLog" :: instanceId :: Nil))) => {
      val log = InstanceLogging.getLocation(console.compota.compotaConfiguration, instanceId)
      Redirect(aws.s3.generateTemporaryURL(log, 60 * 5))
    }

    case GET(Path(Seg("log" :: id :: Nil))) => {
      Redirect(aws.s3.generateTemporaryURL(console.compota.compotaConfiguration.loggingDestination(id), 60 * 5))
    }
...
class Server(compota: Compota) {
  val password = compota.compotaConfiguration.password

  object users extends Users {
    override def auth(u: String, p: String): Boolean = u.equals("nispero") && p.equals(password)
  }

  val console = new Console(compota, Server.this)
  ...
  def start() {
   ...
   // println(keyConf)
    ...
    try {
      Https(443).handler(new ConsolePlan(compota.compotaConfiguration.id, users, console)).start()
    } catch {
      case t: Throwable => {
        println("trying to bind to localhost")
        Https(443, "localhost").handler(new ConsolePlan(compota.compotaConfiguration.id, users, console)).start()
      }
    }
